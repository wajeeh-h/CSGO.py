    ...
    for i in range(1, 16):  # Loop through all entities (max=32)
        ...
        if entity:
            ...
            if player_team == entity_team:
                if settings.settings_dict.get("team_glow_enabled"):
                    write_glow(csgo, glow_manager, entity_glow, settings.settings_dict.get("glow_colour_team"))

            elif player_team != entity_team:
                if settings.enemy_glow_enabled:
                    write_glow(csgo, glow_manager, entity_glow, settings.settings_dict.get("glow_colour_enemy"))
